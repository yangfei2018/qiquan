...
import com.cjy.qiquan.utils.AppContext;
...
public class SignHelper {
	...
	public static void initProject() {
		...
		// 项目ID(应用ID)
		proCfg.setProjectId("1111565196");// 1111563517
		// 项目Secret(应用Secret)
		proCfg.setProjectSecret("412c235fc3f2e0675b030d80aafc5442"); // 95439b0863c241c63a861b87d1e647b7
		// 开放平台地址
		proCfg.setItsmApiUrl("http://openapi.tsign.cn:8080/tgmonitor/rest/app!getAPIInfo2");
	...
	public static void fillPdf(String htname, User user, PartnerBankInfo partner, Map txtFields) {

		String srcPdfFile = AppContext.class.getClassLoader().getResource("pdftemplate/" + htname + ".pdf").getPath();

		String fillPdfFile = Constant.APP_UPLOAD_PATH + "signed/" + user.getUserId() + "_" + htname + ".pdf";
		SignPDFFileBean signPDFFileBean = new SignPDFFileBean();
		// 待签署文档本地路径
		signPDFFileBean.setSrcPdfFile(srcPdfFile);
		// 签署后文档本地路径
		signPDFFileBean.setDstPdfFile(fillPdfFile);
		// 文档名称，e签宝签署日志对应的文档名，若为空则取文档路径中的名称
		signPDFFileBean.setFileName(user.getUserId() + "_" + htname);
		// 文档编辑密码，当目标PDF设置权限密码保护时必填 */
		signPDFFileBean.setOwnerPassword(null);

		// DocService docService = DocServiceFactory.instance();
		// GetFieldsResult r = docService.getPdfAllFields(srcPdfFile);

		PdfDocumentService pdfDocumentService = PdfDocumentServiceFactory.instance();

		FileCreateFromTemplateResult res = pdfDocumentService.createFileFromTemplate(signPDFFileBean, false, txtFields);

	}

	public static void signPdf(String htname, User user, PartnerBankInfo partner, Map txtFields) {

		String srcPdfFile = AppContext.class.getClassLoader().getResource("pdftemplate/" + htname + ".pdf").getPath();

		String fillPdfFile = Constant.APP_UPLOAD_PATH + "signed/" + user.getUserId() + "_" + htname + ".pdf";

		String signedPdfFile = Constant.APP_UPLOAD_PATH + "signed/" + user.getUserId() + "_" + htname + "_signed.pdf";
		...
		FileCreateFromTemplateResult res = pdfDocumentService.createFileFromTemplate(signPDFFileBean, false, txtFields);
		String sealData = user.getSealData();
		if (CommonUtils.isBlankOrNull(sealData)) {
			// 创建个人印章
			AddSealResult userPersonSealData = SignHelper.addPersonTemplateSeal(user.getSignAccountId());
			UserService.instance().updateColById("f_sealData", userPersonSealData.getSealData(), user.getUserId());
			sealData = userPersonSealData.getSealData();
		}

		FileDigestSignResult pSignResult = SignHelper.userPersonSignByFile(res.getDstPdfFile(), signedPdfFile,
				user.getSignAccountId(), sealData, "签字");

		// 企业印章
		String ogrnSealData = partner.getSealData();
		if (CommonUtils.isBlankOrNull(ogrnSealData)) {
			AddSealResult userOrganizeSealData = SignHelper.addOrganizeTemplateSeal(partner.getSignAccountId());
			PartnerService.instance().updatePartnerBankInfoColById("f_sealData", userOrganizeSealData.getSealData(),
					partner.getId());
			ogrnSealData = userOrganizeSealData.getSealData();
		}

		FileDigestSignResult oSignResult = SignHelper.userPersonSignByFile(signedPdfFile, signedPdfFile,
				partner.getSignAccountId(), ogrnSealData, htname.equals("合伙协议补充协议") ? "签章" : "盖章");
	...
	public static FileDigestSignResult platformSignByFile(String srcPdfFile, String signedPdfFile, String key) {
		// 设置文件签署的PDF文档信息
		SignPDFFileBean signPDFFileBean = setSignPDFFileBean(srcPdfFile, signedPdfFile);
		PosBean posBean = setKeyPosBean(key, 110, 0, 90);
		// 设置签署类型为 关键字签章
	...
	public static GetFieldsResult getPdfAllFields(String srcPath) {
		DocService docService = DocServiceFactory.instance();
		return docService.getPdfAllFields(srcPath);
	}
	...
	public static FileDigestSignResult userPersonSignByFile(String srcPdf, String signedPdf, String accountId,
			String sealData, String key) {

		// 设置文件流签署的PDF文档信息
		SignPDFFileBean signPDFFileBean = setSignPDFFileBean(srcPdf, signedPdf);

		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean(key, 100, 0, 159);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;
	...
	public static GetAccountProfileResult getAccountInfoByIdNo(final String idNo, final LicenseQueryType type) {
		AccountService accountService = AccountServiceFactory.instance();
		return accountService.getAccountInfoByIdNo(idNo, type);
	}

	public static String addPersonAccount(User user) {
		...
		// 姓名
		personBean.setName(user.getRealName());
		// 身份证号/护照号
		personBean.setIdNo(user.getIdCard());
	...
	public static String addOrganizeAccount(PartnerBankInfo partner) {
		OrganizeBean organizeBean = new OrganizeBean();
		// 邮箱地址,可空
		// organizeBean.setEmail(null);
		// 手机号码,可空
		// organizeBean.setMobile(partner.getmo());
		// 企业名称
		organizeBean.setName(partner.getCompanyName());// 天之云信息科技有限公司为公共使用，请不要修改
		...
		organizeBean.setOrganCode(partner.getOrganCode());// 52227058XT51M4AL62为公共使用，请不要修改
		// 公司地址,可空
		// organizeBean.setAddress(partner.get);
		// 经营范围,可空
		...
		// 代理人姓名，当注册类型为1时必填
		organizeBean.setAgentName("");
		// 代理人身份证号，当注册类型为1时必填
		organizeBean.setAgentIdNo("");

		// 法定代表姓名，当注册类型为2时必填
		organizeBean.setLegalName(partner.getRealName());
		// 法定代表人归属地,0-大陆，1-香港，2-澳门，3-台湾，4-外籍，默认0
		organizeBean.setLegalArea(0);
		// 法定代表身份证号/护照号，当注册类型为2时必填
		organizeBean.setLegalIdNo(partner.getIdCard());
	...
	public static void updatePersonAccount(String accountId, String realName, String mobile) {

		UpdatePersonBean updatePersonBean = new UpdatePersonBean();
		updatePersonBean.setMobile(mobile);
		updatePersonBean.setName(realName);
		AccountService accountService = AccountServiceFactory.instance();
		Result result = accountService.updateAccount(accountId, updatePersonBean, null);
		if (0 != result.getErrCode()) {
			LOG.info("更新个人账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
	...
	public static void updateOrganizeAccount(String accountId, String name, String mobile) {

		UpdateOrganizeBean updateOrganizeBean = new UpdateOrganizeBean();
		updateOrganizeBean.setMobile(mobile);
		updateOrganizeBean.setName(name);
