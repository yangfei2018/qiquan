...
<body>
								...
								<div class="tabs-container">
									<ul class="nav nav-tabs">
										<%-- <li <c:if test="${status==-1 }">class="active"</c:if>><a
											href="gm/tradelist?status=-1"> 全部</a></li> --%>
										<li <c:if test="${status==0 }">class="active"</c:if>><a
											href="gm/tradelist?status=0"> 未报价</a></li>
										<li <c:if test="${status==1 }">class="active"</c:if>><a
											href="gm/tradelist?status=1"> 已报价</a></li>
									</ul>
													...
													<tbody>
														<c:forEach items="${page.list }" var="item">
															<tr>
																...
																<td>
																<c:if test="${status==0 }">
																	<button onClick="baojia('${item.tradeNo}');"
	...
	<script>
		...
		function excel() {
			gohref("gm/tradelist?status=${status}&excel=1")
		}
