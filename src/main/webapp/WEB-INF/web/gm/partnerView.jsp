						...
						<form class="form-horizontal">
								...
								<div class="form-group">
									...
									<div class="col-lg-10">
										<input type="text" class="form-control" name="password"
											placeholder="请设置代理商登录密码，建议大于4个字符。" />
									</div>
								</div>
							...
							<div class="form-group">
								...
								<div class="col-lg-10">
									<c:choose>
										<c:when test="${edit==0 }">
											<p class="form-control-static">${partner.idCard }</p>
										</c:when>
										<c:when test="${edit==1 }">
											<input type="text" class="form-control" name="idCard"
												value="${partner.idCard }" />
										</c:when>
									</c:choose>
							...
							<div class="form-group">
								<label class="col-lg-2 control-label">组织机构代码号</label>
								<div class="col-lg-10">
									<c:choose>
										<c:when test="${edit==0 }">
											<p class="form-control-static">${partner.organCode }</p>
										</c:when>
										<c:when test="${edit==1 }">
											<input type="text" class="form-control" name="organCode"
												value="${partner.organCode }" />
										</c:when>
									</c:choose>
							...
							<div class="form-group">
								<label class="col-lg-2 control-label">银行卡户名</label>
								<div class="col-lg-10">
									<c:choose>
										<c:when test="${edit==0 }">
											<p class="form-control-static">${partner.bankhm }</p>
										</c:when>
										<c:when test="${edit==1 }">
											<input type="text" class="form-control" name="bankhm"
												value="${partner.bankhm }" />
										</c:when>
									</c:choose>
									...
									<c:choose>
										...
										<c:when test="${edit==1 }">
											<input type="text" class="form-control" name="bankCardNo"
												placeholder="请输入银行卡号" value="${partner.bankCardNo }" />
										</c:when>
									</c:choose>
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-sm-4 col-sm-offset-2">
									<button class="btn btn-danger" type="button"
												onclick="resetpwd(this)">重置密码</button>
				...
				<c:choose>
					<c:when test="${edit==0 }">
						<div class="pd">
							...
							<c:if test="${!empty partner.sealData }">
								<h4>企业签名印章</h4>
								<p class="text-center">
									<img class="thumbnail"
										src="data:image/png;base64,${partner.sealData}"
										alt="数字签名印章">
								</p>
							</c:if>
						</div>
	...
	<script>
		...
		function resetpwd(obj){
			...
			swal({
				...
				$.ajax({
					url : 'gm/json/resetPasswordPartner',
					type : 'post',
					dataType : "json",
