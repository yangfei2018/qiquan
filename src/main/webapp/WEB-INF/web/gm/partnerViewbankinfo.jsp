						...
						<div class="row">
							<table class="table table-striped table-bordered table-hover"
								...
								<thead>
									<tr>
										<th>公司</th>
										<th style="width:10%">普通合伙人</th>
										<th style="width:10%">法人名称</th>
										<th style="width:10%">法人身份证</th>
										<th style="width:10%">组织机构代码</th>
										<th>注册地址</th>
										<th>银行</th>
										<th>卡号</th>
										<th>已关联用户数</th>
									...
									<c:forEach items="${bankInfoList }" var="item">
										<tr class="gradeX" id="row_${item.id }">
											...
											<td data-type="canEdit"><span>${item.realName }</span>
												...
												style="display: none" /></td>	
											<td data-type="canEdit"><span>${item.idCard }</span>
												<input type="text" name="idCard"
												value="${item.idCard }" class="form-control no-border"
												style="display: none" /></td>
											<td data-type="canEdit"><span>${item.organCode }</span>
												<input type="text" name="organCode"
												value="${item.organCode }" class="form-control no-border"
												style="display: none" /></td>		
	...
	<script>
		...
		function addRow(id) {
			if (!editTag) {
				...
				type="text" name="hehuoName" value=""\
				class="form-control no-border" /></td>\
				<td data-type="canEdit"><span style="display:none"></span> <input\
				type="text" name="realName" value=""\
				class="form-control no-border" /></td>\
				<td data-type="canEdit"><span style="display:none"></span> <input\
				type="text" name="idCard" value=""\
				class="form-control no-border" /></td>\
				<td data-type="canEdit"><span style="display:none"></span> <input\
				type="text" name="organCode" value=""\
				class="form-control no-border" /></td>\
				<td data-type="canEdit"><span style="display:none"></span> <input\
		...
		function saveRow(id) {
			...
			var idCard =  $("#row_" + id).find('input[name="idCard"]').val();
			var organCode =  $("#row_" + id).find('input[name="organCode"]').val();
