...
<%@page import="com.cjy.qiquan.utils.CommonUtils"%>
		...
		<div class="wrapper wrapper-content animated fadeInRight">
									...
									<tbody>
										<c:forEach items="${goodsList }" var="item">
											<tr class="gradeX">
												...
												<td>${item.code }</td>
												<td>${item.name }</td>
												<td align="center">${item.feilv_15 }</td>
												<td align="center">${item.feilv_30 }</td>
												<td align="center">
													<c:choose>
														<c:when test="${item.feilv_15Changed==0}">
														
														</c:when>
														<c:when test="${item.feilv_15Changed!=item.feilv_15}">
															<span style="color: red"> ${item.feilv_15ChangedFormat }
															</span>
														</c:when>
														<c:otherwise>
															${item.feilv_15ChangedFormat }
														</c:otherwise>
															</c:choose></td>
												<td align="center">
													<c:choose>
														<c:when test="${item.feilv_30Changed==0}">
														</c:when>
														<c:when test="${item.feilv_30Changed!=item.feilv_30}">
															<span style="color: red"> ${item.feilv_30ChangedFormat }
															</span>
														</c:when>
														<c:otherwise>
												${item.feilv_30ChangedFormat }
												</c:otherwise>
													</c:choose></td>
												<td>${item.feilv_15_timeChanged }</td>
												<td>${item.feilv_30_timeChanged }</td>	
			...
			<div class="row">
				<div class="col-xs-6" style="text-align: center">
					<button class="btn btn-default btn-large" type="button" onclick="cancelUpdate()">取消</button>
				</div>
				<div class="col-xs-6" style="text-align: center">
					<button class="btn btn-primary btn-large" type="button" onclick="updateData('${tmpKey}')">确认更新</button>
				</div>
			</div>
		</div>
	</div>
